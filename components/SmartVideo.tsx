...
export function SmartVideo({ manifest, poster, baseUrl }: SmartVideoProps) {
    ...
    useEffect(() => {
        if (videoRef.current && manifest) {
            controllerRef.current = new MSEController(videoRef.current, manifest, baseUrl);
            ...
            setDuration(manifest.duration);
            setAvailableQualities(controllerRef.current.getAvailableQualities());
            setCurrentQuality(controllerRef.current.getCurrentQuality());
        }
                            ...
                            <div className="absolute bottom-full mb-2 right-0 bg-black/90 rounded overflow-hidden">
                                {availableQualities.map(quality => (
                                    <button
                                        ...
                                        className={`block w-full text-xs px-4 py-2 text-left hover:bg-white/20 transition-colors ${quality === currentQuality ? 'bg-primary text-white' : ''
                                            }`}
                                    >
                                        {quality}
                                    </button>
